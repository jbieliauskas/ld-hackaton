<template>
  <div>
    <p>
      <img
        width="80"
        height="80"
        src="data:image/png;base64,"
      >
    </p>
    <div class="center">
      <div class="keyboard">
        <li class="letter" @click="action(1)">1</li>
        <li class="letter" @click="action(2)">2</li>
        <li class="letter" @click="action(3)">3</li>
        <li class="letter clearl">4</li>
        <li class="letter">5</li>
        <li class="letter">6</li>
        <li class="letter clearl">7</li>
        <li class="letter">8</li>
        <li class="letter">9</li>
        <li class="letter clearl">*</li>
        <li class="letter">0</li>
        <li class="letter">#</li>
        <li class="endCall clearl">
          <img
            width="80"
            height="80"
            src="https://cdn3.iconfinder.com/data/icons/cosmo-color-multimedia-2/40/hang-512.png"
            @click="stopCall"
          >
        </li>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "PhoneCall",
  callSound: null,
  textSound: null,
  props: {
    mp3: String
  },
  methods: {
    playSound(mp3) {
      if (mp3) {
        this.callSound = new Audio(
          "https://www.myinstants.com/media/sounds/skype-incoming.mp3"
        );
        this.callSound.play();
        this.callSound.addEventListener("ended", function() {
          this.textSound = new Audio(mp3);
          this.textSound.play();
        });
      }
    },
    action(key) {
      switch (key) {
        case 1:
          alert("Ok");
          break;
        case 2:
          alert("Bye");
          break;
        case 3:
          alert("Don't know");
          break;
        default:
          alert("No action");
      }
    },
    stopCall() {
      if (this.textSound) {
        this.textSound.pause();
      } else {
        this.callSound.pause();
      }
    }
  },
  mounted() {
    this.playSound(this.mp3);
  }
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
h3 {
  margin: 40px 0 0;
}
ul {
  list-style-type: none;
  padding: 0;
}
li {
  display: inline-block;
  margin: 0 10px;
}
a {
  color: #42b983;
}
.keyboard {
  margin: 0;
  padding: 0;
  list-style: none;
}
.keyboard li {
  float: left;
  margin: 0 5px 5px 0;
  width: 60px;
  height: 60px;
  font-size: 24px;
  line-height: 60px;
  text-align: center;
  background: #fff;
  border: 1px solid #e5e5e5;
  border-radius: 5px;
}
.clearl {
  clear: left;
}
.keyboard li:hover {
  position: relative;
  top: 1px;
  left: 1px;
  border-color: #f9f9f9;
  cursor: pointer;
}
.keyboard .endCall {
  width: 90px;
  margin-left: 45px;
}
.center {
    display: flex;
    justify-content: center;
}
</style>
